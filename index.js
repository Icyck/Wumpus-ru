...
if(cmd === `${prefix}remrole`){
 ...
 if(args[0] == "commands"){
    ...
    .addField("Mute - Заглушить пользователя", `${prefix}mute <@user> <reason>`)
    .addField("Unmute - Снять заглушку с пользователя", `${prefix}unmute <@user>`)
    .addField("Server id - Индефикатор сервера ", `${prefix}serverid`)
    .addField("Report - Отчёт", `${prefix}report`)
    .addField("User id - Работает", `${prefix}userid <@user>`)
    .addField("Warn - Предупредить пользователя", `${prefix}warn <@user> <Reason>`)
    .addField("Warn level - В разработке", `${prefix}warnlevel <@user>`)
    .addField("Kick - Изгнать пользователя", `${prefix}kick`)
    .addField("Ban - Забанить пользователя", `${prefix}ban <@user>`)
    .addField("Clear - Очистить чат", `${prefix}clear `)
    .addField("Setrole - Добавить роль", `${prefix}setrole <@user> <role>`)
    .addField("Remove role  - Убрать роль с пользователя", `${prefix}remrole <@User> <role>`)
